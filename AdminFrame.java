    ...
    private void loadShowtimes(JPanel showtimeListPanel) {
        try {
            ...
            for (Showtime showtime : showtimes) {
                ...
                String movieTitle = (showtime.getMovieTitle() != null && !showtime.getMovieTitle().isEmpty()) ? showtime.getMovieTitle() : "Chưa có phim";
                String status = (showtime.getStatus() != null && !showtime.getStatus().isEmpty()) ? showtime.getStatus() : "Không chiếu";
                SimpleDateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
                JLabel showtimeLabel = new JLabel(String.format("%02d - %s - Phim: %s - Trạng thái: %s - Thời gian: %s",
                        showtime.getShowtimeID(), showtime.getRoomName(), movieTitle, status, sdf.format(showtime.getShowDate())));
                ...
                if (status.equals("Đã chiếu xong")) {
                ...
                hideButton.addActionListener(e -> {
                    try {
                        showtimeBUS.updateShowtimeStatus(showtime.getShowtimeID(), "Ẩn");
                        loadShowtimes(showtimeListPanel);
                        JOptionPane.showMessageDialog(this, "Ẩn suất chiếu thành công");
                    } catch (SQLException ex) {
                        JOptionPane.showMessageDialog(this, "Không thể ẩn suất chiếu: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
                    }
                });
            ...
            JOptionPane.showMessageDialog(this, "Không thể tải danh sách suất chiếu: " + ex.getMessage(), "Lỗi", JOptionPane.ERROR_MESSAGE);
        }
    }
